...
class CustoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(CustoForm, self).__init__(*args, **kwargs)
    ...
    def clean(self):
        ...
        if (Custo.objects.filter(estabelecimento_id=self.estabelecimentoid, nome=self.cleaned_data['nome']).exists() and
            Custo.objects.get(estabelecimento_id=self.estabelecimentoid,
                              nome=self.cleaned_data['nome']).id != self.instance.id):

            raise forms.ValidationError("Já existe um Custo com o nome: " + self.cleaned_data['nome'])
